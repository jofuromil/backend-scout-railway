...
export default function KardexScoutDirigente() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const userRes = await axios.get(`/api/users/kardex-scout/${scoutId}`, config);
        setUsuario(userRes.data);

        const objRes = await axios.get(`/api/Objetivo/resumen-scout?usuarioId=${scoutId}`, config);
        setResumenObjetivos(objRes.data);

        const espRes = await axios.get(`/api/especialidades/resumen-scout/${scoutId}`, config);
        setEspecialidades(espRes.data);

      } catch (err) {
        console.error("Error cargando Kardex del scout:", err);
      }
    };
  ...
  if (!usuario) return <p className="p-4">⏳ Cargando Kardex del Scout...</p>;
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-6">
        <h1 className="text-2xl font-bold mb-4">📋 Kardex del Scout</h1>
        ...
        <section className="mb-6">
          ...
          <p><strong>Grupo Scout:</strong> {usuario.unidad?.grupoScout || "-"}</p>
